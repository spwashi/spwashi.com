# GROK.md — Grok adapter for spwashi.com
...
Sense: `npm run wonder`, `npm run spw:lattice`, `npm run audit:copy:accessor`. Explore/plan do not write — use spawn_subagent `plan` or `explore` (read-only). Then `npm run check:local`.
