...
test('settings apply stays off the UI module graph', async () => {
  const [barrel, coreCatalog, featureCatalog, engine, ui] = await Promise.all([
    ...
    readFile(path.join(ROOT, 'public/js/interface/site-settings-ui.js'), 'utf8'),
  ]);
  ...
  assert.match(featureCatalog, /id:\s*'settings-page'[\s\S]*import\('\.\.\/\.\.\/interface\/site-settings-ui\.js'\)/);
  ...
  assert.match(engine, /import\('\/public\/js\/interface\/site-settings-ui\.js'\)/);
