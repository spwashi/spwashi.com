...
type RuntimeContractReport = {
  ...
  errors: string[];
  inspectContracts: number;
  jsEcology: JsModuleEcologyRow[];
  jsEcologyNotes: string[];
...
export type JsModuleEcologyRow = {
  file: string;
  moduleExport: boolean;
  inspectContract: boolean;
  initExport: boolean;
  catalogLoad: boolean;
  composeExport: boolean;
  catalogIds: string[];
  seat: JsModuleEcologySeat;
};

export function seatJsModuleEcology(row: Omit<JsModuleEcologyRow, 'seat' | 'catalogIds'> & { catalogIds?: string[] }): JsModuleEcologySeat {
  if (row.catalogLoad && row.moduleExport) return 'catalog-export';
  if (row.catalogLoad && row.initExport) return 'catalog-init-only';
  if (row.catalogLoad) return 'catalog-unresolved';
  if (row.moduleExport && !row.composeExport) return 'export-unwired';
  if (row.initExport && !row.composeExport && !row.inspectContract) return 'unwired-init';
  if (row.composeExport) return 'compose';
  return 'helper';
}

export function recommendJsModuleEcology(row: JsModuleEcologyRow): string | null {
  if (row.seat === 'helper' || row.seat === 'compose' || row.seat === 'catalog-export') return null;
...
async function collectJsModuleEcology(catalogIdsByFile: Map<string, string[]> = new Map()): Promise<{
  rows: JsModuleEcologyRow[];
  recommendations: string[];
  document: string;
}> {
  const files = (await collectJsFilesUnder(PUBLIC_JS_DIR))
    .filter((file) => !file.startsWith('typed/') && !file.startsWith('generated/'));
  const catalogLoads = new Set<string>();
  ...
  for (const family of ['core.js', 'feature.js', 'region.js', 'enhancement.js']) {
    ...
    for (const match of source.matchAll(CATALOG_LOAD_IMPORT_RE)) {
      ...
      if (!resolved.startsWith(PUBLIC_JS_DIR) || resolved.includes(`${path.sep}types${path.sep}`)) continue;
  ...
  const composeExports = new Set(
    [...composeSource.matchAll(COMPOSE_FROM_RE)].map((match) => match[1].replace(/^\.\//, '')),
  );

  const rows: JsModuleEcologyRow[] = [];
  ...
  const seats: Record<JsModuleEcologySeat, string[]> = {
    'catalog-export': [],
    'catalog-init-only': [],
    'catalog-unresolved': [],
    'unwired-init': [],
  ...
  const catalog: Record<string, { file: string; seat: JsModuleEcologySeat }> = {};
  ...
  for (const key of Object.keys(seats) as JsModuleEcologySeat[]) {
    seats[key].sort();
  }

  const document = `${JSON.stringify({ catalog, seats }, null, 2)}\n`;
  const recommendations = rows.map(recommendJsModuleEcology).filter((item): item is string => Boolean(item));
...
export function moduleSourceUsesContextCleanup(source = ''): boolean {
...
export function moduleSourceRegistersDuplicateCleanup(source = ''): boolean {
  ...
  if (!moduleSourceUsesContextCleanup(text)) return false;
...
function validateModule(
  ...
  if (loadedSource && moduleSourceUsesContextCleanup(loadedSource)) {
    errors.push(
      `${label} calls ctx.addCleanup; loader-mounted modules return a cleanup handle and must not register teardown on the context stack.`,
    );
  }
...
export async function collectRuntimeContractReport(): Promise<RuntimeContractReport> {
  ...
  const catalogIdsByFile = new Map<string, string[]>();
  for (const module of modules) {
    if (!module.importPath || !module.id) continue;
    const absoluteImport = importPathToAbsolute(module.importPath);
    if (!absoluteImport.startsWith(PUBLIC_JS_DIR)) continue;
...
export async function main(): Promise<void> {
  ...
  const seatCount = (seat: JsModuleEcologySeat) => ecology.filter((row) => row.seat === seat).length;
