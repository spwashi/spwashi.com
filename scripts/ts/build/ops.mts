...
function applyLocalDefaults(options: Pick<BuildOptions, 'imageCheck' | 'sitemap' | 'catalog' | 'fingerprintAssets'>): void {
...
export function createLogger(options: Pick<BuildOptions, 'quiet'>): BuildLogger {
  return {
    info(message: string) {
...
export function assertSafeOutputDir(outDir: string): string {
...
export function shouldExcludeRepoPath(repoPath: string, outputRelativePath = 'dist'): boolean {
...
export async function copyFileOrRender(srcPath: string, dstPath: string): Promise<Exclude<CopyResult, 'skipped-directory' | 'skipped-missing' | 'skipped-special'>> {
...
export function listSourceRepoPaths(options: Pick<BuildOptions, 'outDir'>): string[] {
...
export async function checkImageRedundancy(sourcePaths: string[]): Promise<string[][]> {
  const imagePaths = sourcePaths.filter((repoPath) => {
    if (!repoPath.startsWith('public/images/')) return false;
    const extension = path.posix.extname(repoPath).toLowerCase() as (typeof IMAGE_EXTENSIONS)[number];
    return IMAGE_EXTENSIONS.includes(extension);
  });

  const pathsBySize = new Map<string, string[]>();
...
export function logDuplicateImages(duplicateGroups: string[][], logger: BuildLogger): void {
...
export async function copyTrackedPath(repoPath: string, options: BuildOptions): Promise<CopyResult> {
...
export async function copyRepo(sourcePaths: string[], options: BuildOptions, logger: BuildLogger): Promise<CopyStats> {
  let nextIndex = 0;
  let completed = 0;
