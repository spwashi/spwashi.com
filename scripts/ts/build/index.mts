...
async function injectBootModulePreloads(outDir: string, hrefs: string[]): Promise<number> {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (!SITE_SCRIPT_RE.test(source) || source.includes('data-spw-boot-chunk="true"')) return;
 ...
 */
async function stripPublicCssComments(
  outDir: string,
  logger: BuildLogger,
): Promise<{ files: number; beforeBytes: number; afterBytes: number }> {
  ...
  logger.info(
    `[build] stripped comments from ${cssFiles.length} css files: ${beforeBytes} → ${afterBytes} bytes `
...
async function minifyPublicJsModules(
  ...
  async function minifyOne(filePath: string): Promise<void> {
    ...
    const chunk = bundle.output.find((out): out is OutputChunk => out.type === 'chunk');
    if (chunk) {
...
async function hashAndRewritePublicAssets(
  ...
  if (activeRewrites.length > 0) {
    ...
    let cursor = 0;
    async function rewriteWorker(): Promise<void> {
    ...
    await Promise.all(
      Array.from({ length: Math.min(rewritePoolLimit, rewriteTargets.length) }, () => rewriteWorker()),
    );
  }
