...
export type JsonContractReport = {
  checked: number;
  errors: string[];
  warnings: string[];
};
...
type JsonValidatorModule = {
  formatJsonIssues: (issues: { path: string; message: string }[]) => string[];
  parseJsonValue: (text: string, label?: string) => unknown;
  validateMediaPublishingConfig: (value: unknown) => { ok: boolean; issues: { path: string; message: string }[] };
  validatePromoWonderFeed: (value: unknown) => { ok: boolean; issues: { path: string; message: string }[] };
};

async function loadJsonValidators(): Promise<JsonValidatorModule> {
  const modulePath = path.join(ROOT_DIR, 'public/js/typed/json-feeds.js');
  return import(pathToFileURL(modulePath).href) as Promise<JsonValidatorModule>;
}

export async function collectJsonContractReport(): Promise<JsonContractReport> {
  const {
    formatJsonIssues,
    parseJsonValue,
    validateMediaPublishingConfig,
    validatePromoWonderFeed,
  } = await loadJsonValidators();
