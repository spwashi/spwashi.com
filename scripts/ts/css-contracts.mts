...
function getCssTopLevelDir(rootRelativePath: string): string | null {
  const parts = rootRelativePath.replace(/^\/+/, '').split('/');
  if (parts[0] !== 'public' || parts[1] !== 'css') return null;
  return parts[2]?.endsWith('.css') ? null : parts[2] || null;
}

function expectedLayerForCssPath(rootRelativePath: string): string | null {
 ...
 */
async function collectDeferredRuntimeCss(errors: string[]): Promise<Set<string>> {
  ...
  if (registrations.length !== invocationCount) {
    errors.push(
      'Every ensureDeferredStyles() call must use literal id and /public/css/*.css arguments '
  ...
  while (pending.length) {
    ...
    if (!href || discovered.has(href)) continue;
    discovered.add(href);
    ...
    try {
      ...
      for (const imported of deferredImports) {
        ...
        if (!imported.layer) {
          ...
          errors.push(
            `${imported.file} lives under public/css/${getCssTopLevelDir(imported.file)}/ and must be imported `
            + `by deferred stylesheet ${href} in layer(${expectedLayer}), not layer(${imported.layer}).`,
          );
        }
        if (!discovered.has(imported.file)) pending.push(imported.file);
      }
    } catch {
...
function hasTopFileContract(source: string): boolean {
  if (source.startsWith('@layer ')) return true;
...
function unwrapOuterWhere(selector: string): string | null {
  const trimmed = selector.trim();
  if (!trimmed.startsWith(':where(') || !trimmed.endsWith(')')) return null;
  let depth = 0;
...
function subjectIsRootElement(selector: string): boolean {
  const inner = unwrapOuterWhere(selector);
  if (inner !== null) return subjectIsRootElement(inner);
  return splitTopLevel(selector).some((part) => {
    const partInner = unwrapOuterWhere(part);
    if (partInner !== null) return subjectIsRootElement(partInner);
...
function hasZeroSpecificity(selector: string): boolean {
  ...
  if (/:(?!:)(?!where)[\w-]/.test(stripped)) return false;
  return !/[a-zA-Z]/.test(stripped.replace(/[\s,>+~()]/g, ''));
}

const ROOT_BASE_SELECTOR = /^(:root|html|:root\s*,\s*html|html\s*,\s*:root)$/;

function findDeadRootStateRules(source: string): { line: number; selector: string; property: string }[] {
  const masked = source.replace(/\/\*[\s\S]*?\*\//g, (comment) => comment.replace(/[^\n]/g, ' '));
  const rules: { selector: string; properties: string[]; line: number }[] = [];
  const rulePattern = /([^{}@][^{}]*)\{([^{}]*)\}/g;
  ...
  for (const rule of rules) {
    if (!ROOT_BASE_SELECTOR.test(rule.selector.replace(/\s+/g, ' '))) continue;
    for (const property of rule.properties) baseProperties.add(property);
  }

  const dead: { line: number; selector: string; property: string }[] = [];
  for (const rule of rules) {
    if (ROOT_BASE_SELECTOR.test(rule.selector.replace(/\s+/g, ' '))) continue;
    if (!hasZeroSpecificity(rule.selector) || !subjectIsRootElement(rule.selector)) continue;
    for (const property of rule.properties) {
      if (!baseProperties.has(property)) continue;
      dead.push({
        line: rule.line,
  ...
  for (const absolutePath of cssFiles) {
    ...
    if (!compatibilityWrapper && !isGeneratedBundlePath(rootPath)) {
      for (const dead of findDeadRootStateRules(source)) {
        errors.push(
          `${relativePath}:${dead.line} sets ${dead.property} from a zero-specificity `
