...
import { collectTagAttributes, extractRuntimeArrayLiteral, splitList, stripQueryHash, } from './site-contracts/helpers.mjs';
...
function normalizeLocalUrl(value) {
    ...
    if (decoded.split('/').includes('..') || decoded.includes('\0'))
        return null;
    return decoded || '/';
}
function resolveLocalPath(rootDir, urlPath) {
...
async function assetExists(rootDir, urlPath) {
    return isFile(resolveLocalPath(rootDir, urlPath));
}
async function routeExists(rootDir, route) {
    const pathname = normalizeLocalUrl(route);
    if (!pathname)
        return false;
    if (pathname === '/')
        return isFile(path.join(rootDir, 'index.html'));
...
function decodeArrayString(token) {
    const quote = token[0];
...
function extractArrayValues(source, name, constants) {
...
export function extractServiceWorkerPrecache(source) {
...
export function collectOfflineDocumentDependencies(html) {
    const dependencies = [];
    ...
    for (const attrs of collectTagAttributes(html, 'link')) {
        ...
        if (!rels.some((rel) => dependencyRels.has(rel)))
            continue;
...
export function injectBuildPrecacheAssets(workerSource, dependencies) {
    ...
    const withoutPreviousBuildList = workerSource
        .replace(/const\s+BUILD_PRECACHE_ASSETS\s*=\s*\[[\s\S]*?\];\s*/, '')
    ...
    const buildAssets = uniqueSorted(dependencies
        .map((value) => normalizeLocalUrl(value))
        .filter((value) => Boolean(value))).filter((value) => !existingRequired.has(value));
    ...
    return withRequiredBuildAssets.replace(precacheDeclaration, 'const PRECACHE_URLS = [...new Set([...REQUIRED_PRECACHE_URLS, ...OPTIONAL_PRECACHE_URLS])];');
}
async function readJsonObject(filePath, label, errors) {
    let source = '';
    try {
...
async function readPngDimensions(filePath) {
    try {
        ...
        try {
            ...
            if (bytesRead < 24 || header.subarray(0, 8).toString('hex') !== signature)
                return null;
            return {
...
async function validateManifest(rootDir, precache, errors) {
    ...
    if (!manifestObject)
        return { iconCount: 0, manifest: null };
    ...
    if (!icons.length)
        errors.push('Web manifest must define at least one icon.');
    let hasAny192 = false;
    let hasAny512 = false;
    ...
    for (const [index, icon] of icons.entries()) {
        ...
        if (!precache.has(src))
            errors.push(`Service worker precache is missing manifest icon ${src}.`);
        ...
        if (purpose.includes('any') && sizes.includes('192x192'))
            hasAny192 = true;
        if (purpose.includes('any') && sizes.includes('512x512'))
            hasAny512 = true;
        ...
        if (icon.type === 'image/png' && iconPath) {
            ...
            if (!dimensions) {
                ...
                errors.push(`${label} declares sizes=${sizes.join(' ')} but file is ${dimensions.width}x${dimensions.height}: ${src}`);
            }
        }
    }
    if (!hasAny192)
        errors.push('Web manifest must include a 192x192 purpose=any icon.');
    if (!hasAny512)
        errors.push('Web manifest must include a 512x512 purpose=any icon.');
    if (!hasMaskable)
        errors.push('Web manifest must include a maskable icon.');
...
async function validateBuildAssetManifest(rootDir, workerPrecache, errors) {
    const manifestObject = await readJsonObject(path.join(rootDir, 'asset-manifest.json'), 'build asset manifest', errors);
    if (!manifestObject)
        return;
    const buildManifest = manifestObject;
    ...
    for (const file of catalogHtmlFiles) {
        ...
        for (const removed of removedAssets) {
            if (source.includes(removed)) {
                errors.push(`Generated catalog references removed entry asset ${removed}: ${path.relative(rootDir, file)}`);
            }
        }
    }
}
export async function collectPwaContractReport(options = {}) {
    ...
    if (!precache.routes.length)
        errors.push('Service worker CORE_ROUTES could not be read.');
    if (!precache.assets.length)
        errors.push('Service worker CORE_ASSETS could not be read.');
    for (const route of precache.routes) {
        if (!(await routeExists(rootDir, route)))
            errors.push(`Precache route does not resolve: ${route}`);
    }
    for (const asset of precache.assets) {
        if (!(await assetExists(rootDir, asset)))
            errors.push(`Precache asset does not resolve: ${asset}`);
    }
    ...
    if (!precache.routes.includes('/offline/'))
        errors.push('Service worker precache is missing /offline/.');
    ...
    if (mode === 'dist') {
        ...
        if (!precacheExpression.includes('REQUIRED_PRECACHE_URLS')
...
export function formatPwaContractSummary(report) {
...
export async function assertPwaContract(options = {}) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--mode' && argv[index + 1]) {
            ...
            if (mode !== 'source' && mode !== 'dist')
                throw new Error(`[pwa] unknown mode: ${mode}`);
            options.mode = mode;
            index += 1;
            continue;
        }
