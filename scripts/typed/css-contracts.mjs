...
async function collectStyleSourceFiles() {
    try {
...
async function collectLinkedStylesheets() {
    ...
    for (const routeFile of routeFiles) {
        ...
        for (const attrs of linkTags) {
            if (!attrs.href || !splitList(attrs.rel).includes('stylesheet'))
                continue;
            const href = stripQueryHash(attrs.href);
            if (href.startsWith('/public/css/'))
                stylesheets.add(href);
        }
    }
    return [...stylesheets].sort();
}
function isGeneratedBundlePath(rootPath) {
    return rootPath.startsWith('/public/css/bundles/');
}
function hasInstructionHeader(source, filename) {
    if (filename === 'style-core.css')
        return source.startsWith('@layer ');
    if (filename === 'style.css')
        return source.trimStart().startsWith('/*');
    const firstChunk = source.slice(0, 900);
    return firstChunk.trimStart().startsWith('/*') && (firstChunk.includes(filename)
...
function isCompatibilityWrapper(source, filename) {
    const trimmed = source.trim();
    const wrapperPattern = new RegExp(`^@import\\s+url\\('/public/css/[a-z0-9_-]+/${filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}'\\);$`);
...
export async function collectCssContractReport() {
    ...
    for (const item of imports) {
        ...
        if (item.file === '/public/css/style-core.css')
            continue;
    ...
    for (const file of styleOnlyImports) {
        if (!scopedRouteAndBehaviorFiles.has(file)) {
            errors.push(`${file} is imported by style.css but not assigned to ROUTE_SCOPES or BEHAVIOR_SCOPES `
