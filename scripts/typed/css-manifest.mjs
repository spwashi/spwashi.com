...
export const ROUTE_SCOPES = Object.freeze({
    ...
    'services-care': [
        '/public/css/routes/surfaces/services.css',
        '/public/css/routes/widgets/care-intake.css',
    ],
...
export function resolveCanonicalRouteSurface(surface) {
...
export function isKnownRouteSurface(surface) {
    const normalized = normalizeSpace(surface);
    if (!normalized)
        return false;
    if (normalized in ROUTE_SCOPES)
        return true;
...
export async function estimateCssPayload(options = {}) {
    ...
    const sheets = mode === 'full'
        ? [{ href: FULL_STYLESHEET_HREF, kind: 'full' }]
 ...
 */
export function filterBundleTargets(targets, options = {}) {
    ...
    return selected.filter((target) => {
        if (target.kind === 'core')
            return wantCore && !options.skipCore;
...
export function normalizeCssSourceHref(pathOrHref) {
...
export function isGeneratedCssHref(href) {
    const normalized = normalizeCssSourceHref(href);
    return (normalized.startsWith('/public/css/bundles/')
 ...
 */
export function targetsForSourcePaths(pathOrHrefs, options = {}) {
    const targets = options.targets || listBundleTargets();
    const coreFiles = new Set([...(options.coreSourceHrefs || [])]
    ...
    for (const raw of pathOrHrefs) {
        ...
        if (!href || isGeneratedCssHref(href))
            continue;
        for (const target of targets) {
            if (target.kind === 'core') {
                if (coreFiles.has(href))
...
export function onlyTokensForTargets(targets) {
    const tokens = [];
    for (const target of targets) {
        if (target.kind === 'core')
            tokens.push('core');
        else if (target.kind === 'route')
            tokens.push(`route:${target.scope}`);
        else if (target.kind === 'behavior')
            tokens.push(`behavior:${target.scope}`);
    }
    return tokens;
}
