...
export function parseBrowserModuleImports(source, filename = 'module.js') {
    const syntax = ts.createSourceFile(filename, source, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS);
    const imports = [];
    const record = (expression, kind) => {
        imports.push({
            kind,
...
async function filesUnder(directory, extension, prefix = '') {
    let entries;
    try {
...
export function collectJsFilesUnder(directory) {
    return filesUnder(directory, '.js');
}
function canImportTypedModule(file, kind) {
    if (file.startsWith('kernel/') || file.startsWith('typed/'))
        return true;
...
function resolveBrowserImport(file, specifier) {
    ...
    if (!pathname.startsWith('/public/js/'))
        return { error: 'escapes public/js/' };
    if (!pathname.endsWith('.js'))
        return { error: 'must name an explicit .js module' };
    return { file: pathname.slice('/public/js/'.length) };
}
/** Read-only import and folder ownership checks; never import browser modules. */
export async function collectRuntimeImportReport(rootDir) {
    ...
    const errors = [];
    const typedImportViolations = [];
    const kernelTypedShims = [];
    ...
    for (const file of rootEntrypoints) {
        if (!ROOT_ENTRYPOINTS.has(file))
            errors.push(`public/js/${file} is a root-level JS file; use a recognized ownership directory.`);
    }
    ...
    for (const file of typedFiles) {
        ...
        if (!sourceSet.has(source))
            errors.push(`public/js/${file} has no matching public/ts/${source} source.`);
    }
    for (const source of sourceFiles) {
        const output = `typed/${source.replace(/\.ts$/, '.js')}`;
        if (!fileSet.has(output))
            errors.push(`public/ts/${source} has no matching public/js/${output} output; run npm run build:runtime.`);
    }
    for (const file of files) {
        ...
        for (const entry of imports) {
            ...
            if (file.startsWith('kernel/') && entry.kind === 'export') {
                if (!kernelTypedShims.includes(file))
                    kernelTypedShims.push(file);
                const source = file.slice('kernel/'.length).replace(/\.js$/, '.ts');
                const expected = `typed/${file.slice('kernel/'.length)}`;
                if (!sourceSet.has(source))
                    errors.push(`public/js/${file} re-exports typed output but public/ts/${source} is missing.`);
                if (target !== expected)
                    errors.push(`public/js/${file} must re-export public/js/${expected}, not ${entry.specifier}.`);
            }
        }
    }
    return {
