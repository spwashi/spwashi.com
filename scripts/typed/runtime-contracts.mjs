...
import { ALLOWED_JS_OWNER_DIRECTORIES, collectJsFilesUnder, collectRuntimeImportReport, } from './runtime-contracts/imports.mjs';
...
export function moduleSourceRegistersDuplicateCleanup(source = '') {
    ...
    if (/\breturn\s*\{[\s\S]*?\bcleanup\b/.test(text))
        return true;
    if (/\breturn\s+\w*[Cc]leanup\w*/.test(text))
        return true;
    return false;
}
export function listNamedInitAdapterExports(objectLiteral = '') {
    return [...new Set([...String(objectLiteral).matchAll(NAMED_INIT_ADAPTER_RE)].map((match) => match[1]))];
}
export function moduleSourceExportsName(moduleSource, name, absolutePath = '', depth = 0) {
    if (!name)
        return false;
    ...
    for (const match of starExports) {
        ...
        for (const candidate of candidates) {
            try {
                ...
                if (moduleSourceExportsName(nested, name, candidate, depth + 1))
                    return true;
            }
            catch {
                // try next candidate
 ...
 */
function moduleSourceHasResolvableMount(moduleSource, absolutePath, depth = 0) {
    if (INIT_EXPORT_SOURCE_RE.test(moduleSource))
        return true;
    if (depth >= 2)
        return false;
    const starExports = [...moduleSource.matchAll(/\bexport\s+\*\s+from\s+['"]([^'"]+)['"]/g)];
    for (const match of starExports) {
        ...
        for (const candidate of candidates) {
            try {
                const nested = readFileSync(candidate, 'utf8');
                if (moduleSourceHasResolvableMount(nested, candidate, depth + 1))
                    return true;
            }
            catch {
                // try next candidate
...
function validateModule(module, errors, warnings, recommendations) {
    ...
    if (module.timingArc) {
        ...
        if (timingTokens.length !== 1 || !CONTRACT_TOKEN_RE.test(timingTokens[0])) {
            ...
            recommendations.push(`${label} timingArc "${timingTokens[0]}" should use a known stem `
        ...
        recommendations.push(`${label} is missing timingArc; stamp boot-|immediate-|feature-|visible-|enhance-|idle-|settled-* for mount hygiene.`);
    }
    if (module.timingChunk) {
        ...
        if (chunkTokens.length !== 1 || !CONTRACT_TOKEN_RE.test(chunkTokens[0])) {
            ...
            && !STANDARD_IDLE_CHUNKS.includes(chunkTokens[0])) {
            recommendations.push(`${label} timingChunk "${chunkTokens[0]}" is nonstandard; prefer ${STANDARD_IDLE_CHUNKS.join('|')}.`);
        }
        else if (module.when !== 'idle' && module.timingChunk) {
            recommendations.push(`${label} declares timingChunk but when=${module.when}; idle chunks only order IDLE residue-before-flourish mounts.`);
        }
    }
    ...
    if (module.costClass === 'paint_composite'
        && module.when === 'immediate'
        && module.layer === 'enhancement'
        && module.updates.length >= 3) {
        const roleful = module.updates.filter((token) => (/(?:^|:)(?:structural|flourish|inspect|residue|measure|diagnostic):/.test(token)
            || /^(?:structural|flourish|inspect|residue|measure|diagnostic):/.test(token)));
        if (roleful.length === 0) {
            recommendations.push(`${label} is paint_composite+IMMEDIATE without role: updates; prefix structural/flourish/measure so CSS owners stay aligned.`);
        }
    }
    ...
    if (!/\bmount:\s*\(/.test(module.objectLiteral)) {
        if (!module.importPath) {
            ...
            errors.push(`${label} is missing a mount() adapter and ${module.importPath} has no init*/SPW_MODULE_EXPORT mount for resolveModuleMount.`);
        }
    }
    else {
        ...
        if (namedInits.length) {
            if (!loadedSource) {
                errors.push(`${label} mount adapter calls ${namedInits.join(' / ')} but ${module.importPath || 'its load path'} is unreadable.`);
            }
            else if (!namedInits.some((name) => moduleSourceExportsName(loadedSource, name, loadedPath))) {
                errors.push(`${label} mount adapter calls ${namedInits.join(' / ')} but ${module.importPath} does not export that init.`);
            }
        }
    }
...
export async function main() {
    ...
    const byTimingStem = report.modules.reduce((acc, module) => {
        if (!module.timingArc) {
    ...
    const byIdleChunk = idleModules.reduce((acc, module) => {
