...
import { CORE_BUNDLE_HREF, isKnownRouteSurface, routeBundleHref, } from '../css-manifest.mjs';
...
export function collectLocalFragmentIssues(html, route, features) {
    const ids = new Set();
    ...
    for (const anchor of collectTagAttributes(html, 'a')) {
        ...
        if (routePart && routePart !== '.' && routePart !== './') {
            if (!routePart.startsWith('/'))
                continue;
            if (normalizeInternalRoute(stripQueryHash(routePart)) !== route)
                continue;
        }
        const fragment = decodeFragment(rawFragment);
        if (ids.has(fragment) || runtimeTargets.has(fragment))
            continue;
...
async function parseRouteFile(absoluteFilePath) {
    ...
    if (stylesheetMode === 'scoped' && surface && !isKnownRouteSurface(surface)) {
        errors.push(`data-spw-surface="${surface}" is not in ROUTE_SCOPES/ROUTE_SURFACE_ALIASES; `
    ...
    if (manifestHref)
        requiredHeadAssets.push(manifestHref);
...
export async function buildRouteRuntimeManifest() {
    ...
    const routes = parsedRoutes
        ...
        for (const relatedRoute of route.relatedRoutes) {
            if (!relatedRoute || !relatedRoute.startsWith('/'))
                continue;
            if (/\.[a-z0-9]+$/i.test(relatedRoute))
                continue;
 ...
 */
function runSyntaxCheckBatch(shard) {
    return new Promise((resolve) => {
        const child = spawn(process.execPath, ['--experimental-vm-modules', '--no-warnings', SYNTAX_CHECK_BATCH_SCRIPT], { cwd: ROOT_DIR, stdio: ['pipe', 'pipe', 'pipe'] });
        let stdout = '';
        let stderr = '';
        let settled = false;
        ...
        child.once('close', (status) => {
            if (status !== 0) {
                ...
                if (stderr.trim())
                    console.log(`  syntax-batch: ${normalizeSpace(stderr)}`);
                finish(null);
                return;
            }
            try {
...
function checkSyntaxTargetBySpawn(absolutePath) {
    return new Promise((resolve) => {
...
function shardSyntaxTargets(targets, maxShards) {
...
export async function runSyntaxChecks() {
    ...
    return { concurrency, mode: 'spawn', failures, targets: targets.map(relativeRepoPath) };
}
