...
- After copy-unit / collectible lede / `data-spw-semantic-expression` HTML: `npm run audit:copy:accessor` and `npm run manifest`. Commit `public/data/site-search-index.json` and `public/js/generated/spw-expressions.js` when they move. The search index carries a source stamp; pre-commit and `check-site` fail when it does not match the routes. A matching stamp younger than one hour reuses the shared cache at `.agents/state/runtime/`.
