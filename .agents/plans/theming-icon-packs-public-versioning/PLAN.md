...
## 2026-07-13 — Launcher-mask fix landed; icon design pass queued

**Landed:** `icon-maskable-512.png` was byte-identical to `icon-512.png` — artwork with baked-in rounded corners and a transparent surround declared `purpose: maskable`. Android crops maskable icons to the device tile and fills leftover transparency with black (the observed black border / tile mismatch); iOS composites apple-touch transparency onto black. `scripts/generate-pwa-icons.mjs` (`npm run icons:pwa`, `--check` mode for CI-style drift checks) now derives full-bleed variants from the canonical `icon-512.png`: the artwork's sampled field color extends edge-to-edge, the motif stays inside the maskable safe zone (central 80% circle), and the decorative ring falls outside the launcher crop. Manifest bumped to 0.2.4 and sw cache schema to v4 so devices refetch.

**Queued (design pass, human-led):** the icon system needs cohesive design work across surfaces, approached from the palette/theming tokens rather than per-asset:
- favicon legibility at 16/32px (current 256px ico is the full motif; a simplified glyph — e.g. the sigil or W mark — reads better small)
- route/social thumbnails (og:image coverage is uneven across routes)
- CSS decoration (operator sigils, spec pills, badge chrome) sharing one sizing/palette contract with the launcher icons
- sizing ramp: one source artwork → derived sizes documented in this plan, generated by the icons script, so packs stay re-derivable
- tie-in with theme packs: a future `data-spw-icon-pack` should draw from the same sampled-field + safe-zone discipline the generator encodes

Brand discipline note: derive identity from the working artwork and tokens (ground-up), don't prescribe a new mark ahead of the work.
