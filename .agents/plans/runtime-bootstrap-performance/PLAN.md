...
2026-07-26 continuation: the feature-gated diagnostic `console` moved from
`IMMEDIATE` to the existing `idle-chrome` chunk. It remains available on the
same routes and retains the eager/per-module timing override for QA, but no
longer competes with the first reading posture on the 135 routes that advertise
the console feature.
...
- [x] Phase 3 first data, 2026-09-17 — measured-frame lane: a devtools.timeline trace with stacks on the about route at pocket showed every full style recalculation costing ~300ms against the 1.7MB core bundle, and script forcing them from six places (arrival tokens, scrollY, innerWidth, header rect, medium tokens, viewport box). `kernel/measured-frame.js` now runs every geometry lane in one task after the frame's style pass, read phase then write phase; `kernel/viewport.js` caches the viewport box. Four interleaved A/B runs per side: script-forced passes median 18.5 → 10.5, their cost 7141 → 3657ms, total style recalc 11811 → 10340ms; boot-to-interactive medians within this machine's noise. Observer consolidation (the 27 MutationObserver census below) is still not attempted; the lane is the read-side half of that work.
- [x] Phase 3 second data, 2026-09-20 — scroll frames: a touch scroll on the about route at pocket (two synthesized gestures, ~4000px down and 2200px back, devtools.timeline trace with stacks, 30s settle first so boot stays out of the window) showed the section handle forcing a full style pass per frame by reading scrollY and innerHeight after its own writes inside rAF, the shell forcing another with its own rAF scrollY read, the logo writing `--logo-charge` on every scroll event so every lane that measures after the frame found style dirty, and spatial gravity reading each tracked rect after the previous element's writes. The section handle, the shell scroll band, the logo charge, spatial gravity, and the home section index now read in the measured lane's read phase and write after it; the section handle skips a passive frame whose reading (active index, visibility, cauldron resonance, phase, story, compact, root kin state) matches the last applied one; `writeSectionProgressStyle` writes root tokens only when they move. Two interleaved runs per side: script-forced style passes median 16 → 10, their cost 5608 → 3316ms, total style recalc 10922 → 7929ms, layout 1413 → 498ms, rAF callbacks 4520 → 236ms, main-thread task time 19068 → 15118ms across the ~11s scroll window. A mutation probe in the same trace found `navigation-spells` re-annotating every same-origin link on each settled section (≈750 same-value writes per attribute per scroll); its `setLinkDataset` now writes only what moved (`data-spw-nav-tokenized` mutations 808 → 112; the timing side of that pair was within this machine's noise at two runs and is not claimed). Still inside the scroll window, unaddressed: 13–23 module mounts as VISIBLE/REGION gates open, root pulse writes from `interaction/progression.js` and `pulse-beat-tuner.js` on touch, `data-spw-page-region-*` and `data-spw-spell-path-*` root writes per section, `data-spw-crossref` (~350 writes per scroll), and the shell-reader `spwNavTokenized = 'inspect'` write; the forced passes that remain are lane read phases finding style dirtied by those between the frame and the lane task.
...
**2026-09-03 Phase 3 baseline (not acted on):** a static census found 27 separate `new MutationObserver` call sites and 10 `new IntersectionObserver` call sites across `public/js/`, against 4 modules already on the shared `observeIntersections()` lane in `kernel/browser-primitives.js` (then `runtime-helpers.js`). Consolidation was not attempted this pass — each observer watches a different host/attribute (route/device/host state, nav overflow, settings changes, image capture, etc.), and this plan's own Phase 3 gate ("only if Phase 1+2 data shows observer setup ... as a measurable secondary contributor") has no live profiling data behind it yet. Merging them blind risks real behavioral regressions for a cost this plan hasn't measured. The census is left here as the starting point for whoever picks Phase 3 up with a profiler in hand.
