...
- The operator projections and aligned conventions (attention-field, wonder-vocabulary, semantic-braces, ornament-contract, query-disposition) now share a consistent distribution of operational semantics optimized for emergent cognitive+computational communication.
  - Special refinement for the % operator: curriculum economic analogies (e.g. "memory pressure as %", "platform reach risk as %") can now carry both objective instrumentation and subjective practitioner judgment, captured via workbench plugins and annotation layers. This makes the "measure" thread in the curriculum itself a site for cognitive+computational integration.
...
- **Operators**: Start with existing (`#>`, `?`, `~`, `>`, `@`, `!`) + curriculum-specific `data-spw-curriculum-op` or compound sigils (e.g. `#>liquidity_buffer`). Defer new prefix machinery (shared.js regex + CSS tokens) until dialect proven in 2+ modules. Curriculum Spw library in .spw/ for editor inspectability. All curriculum surfaces must align with the site_operator_projection contracts in `.spw/conventions/operator-site-projection.spw` (ownership, events, claims, and validation per operator). The math-practice-labs slice now serves as the primary concrete example of operator usage in interactive teaching surfaces.
