...
import { createModuleScheduler } from './scheduler.js';
import {
  MODULE_TIMING_STAGES,
  createModuleDisposal,
  mountModuleDefinition,
...
export { MODULE_TIMING_STAGES } from './lifecycle.js';
...
} from '../catalog/describes-contract.js';
import {
...
export const SPW_MODULE_LOADER_CONTRACT = Object.freeze({
  ...
  cleanupOwnership:
    'A catalog unmount(record) adapter owns teardown when declared; it may delegate to record.cleanup. Otherwise the returned cleanup or destroy handle owns teardown. Concurrent releases share one promise and remount waits for teardown.',
  lifecycleEvent:
...
export function createModuleLoader(config = {}) {
  ...
  const { mountImmediateLayer, mountVisibleFeatures, mountInteractionFeatures, mountInvitedFeatures, mountRegionLayer, queueIdleEnhancements, queueSettledEnhancements } = createModuleScheduler({
    mountWhen, regionStates, html, setPageState, pageStates, getRoots, shouldScheduleDefinition, mountDefinition, beginMountBatch, endMountBatch, annotateModuleTrigger,
  });
  ...
  const { pendingUnmounts, unmountModuleRecord, unmountModuleById, unmountAllModules } = createModuleDisposal({
    logger, logRelationships, annotateModuleTarget, scheduleRuntimeTokenUpdate,
  });
...
async function mountDefinition(def, ctx, root = null, index = 0) {
  ...
  const record = ctx.registry.set(recordId, {
    ...
    // Registry-wide destruction and explicit unmount share the same owner.
    dispose: () => unmountModuleRecord(record, ctx),
    refresh: null,
    root,
    mountedAt: null,
    observedAt: null,
