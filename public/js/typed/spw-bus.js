...
const isDomTarget = (value) => (Boolean(value)
    && typeof value.addEventListener === 'function'
    && typeof value.removeEventListener === 'function'
    && typeof value.dispatchEvent === 'function');
const isChargeElement = (value) => ((typeof HTMLElement !== 'undefined'
...
const now = () => Date.now();
const normalizeEventName = (name = '') => (String(name)
    .trim()
    .replace(/^spw:/, ''));
...
class SpwBus {
    ...
    emit(name, detail = {}, options = {}) {
        ...
        const { target = document, bubbles = true, composed = true, cancelable = false, element = null, legacy = true, record = true, } = options;
    ...
    on(name, handler, options = {}) {
        const normalizedName = normalizeEventName(name);
        const { target = document, once = false, passive = true, signal, } = options;
    ...
    recent(filter = null) {
        const records = filter
            ? this.#history.filter((record) => (record.name.includes(filter)
    ...
    #dispatchLegacy(name, detail, options) {
        ...
        if (!legacyName)
            return;
        const { target, bubbles, composed, cancelable, } = options;
    ...
    #resolveElement(detail, explicitElement) {
        if (isChargeElement(explicitElement))
            return explicitElement;
        if (isChargeElement(detail?.element))
            return detail.element;
        if (isChargeElement(detail?.targetElement))
            return detail.targetElement;
        if (isChargeElement(detail?.node))
            return detail.node;
        return null;
    }
...
export { CHARGE_BY_EVENT, DEFAULT_HISTORY_LIMIT, LEGACY, bus, };
export default bus;
