export const REGION_ECOLOGY_FIXTURES = Object.freeze([
    {
        id: 'home-hook',
        label: 'Home hook',
        seat: 'hook',
        selector: '#home-frame',
        specimenRoute: '/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape', 'og'],
        captureValue: 'Entry spine — one breath before the atlas opens.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'Does the hook still hold when the crop is a link preview?',
    },
    {
        id: 'home-entry-loops',
        label: 'Home entry loops',
        seat: 'path',
        selector: '#entry-loops',
        specimenRoute: '/',
        cssOwner: 'public/css/components/cards.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'square'],
        sizeToken: 'measure-card',
        captureValue: 'Four first loops as .spw-card tiles in a live grid.',
        chapter: 'physics',
        liminality: 'entry',
        wonder: 'The public card noun has to pack as a tile, not as a cropped frame.',
    },
    {
        id: 'home-entry-panels',
        label: 'Home entry panels',
        seat: 'cluster',
        selector: '#home-entry-panels',
        specimenRoute: '/',
        cssOwner: 'public/css/components/surfaces.css',
        layoutScenarios: ['pocket', 'fold'],
        socialAspects: ['fit', 'landscape'],
        captureValue: 'Matte request panels with chips, in the home frame.',
        chapter: 'physics',
        liminality: 'threshold',
        wonder: 'A panel is a callout, not a second card. Chips stay inside the tile.',
    },
    {
        id: 'home-cluster',
        label: 'Home reading layers',
        seat: 'cluster',
        selector: '#reading-layers',
        specimenRoute: '/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'square'],
        captureValue: 'Cluster desk where leftover frames wrap at fold.',
        wonder: 'Fold leftover tracks should look like a desk, not a forced two-column main.',
        synergy: { with: '[data-spw-cluster="entry-orientation"]', reason: 'cluster + leftover packing' },
        chapter: 'personality',
        liminality: 'middle',
    },
    {
        id: 'about-hook',
        label: 'About hook',
        seat: 'hook',
        selector: '#about-frame',
        specimenRoute: '/about/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Person-first entry. Hospitality before the years list.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'The person leads. A short crop must not turn identity into a tag list.',
    },
    {
        id: 'about-systems-desk',
        label: 'About systems desk',
        seat: 'cluster',
        selector: '#systems-register',
        specimenRoute: '/about/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape'],
        captureValue: 'Cluster wrapper that leftover-track packing actually spends.',
        wonder: 'At fold, leftover frames wrap inside this desk — the intrigue is the wrap, not a 1fr 1fr main.',
        synergy: { with: '#systems-register', reason: 'hub inside a cluster desk' },
        chapter: 'personality',
        liminality: 'middle',
    },
    {
        id: 'about-years',
        label: 'About years path',
        seat: 'path',
        selector: '#about-years',
        specimenRoute: '/about/',
        cssOwner: 'public/css/routes/surfaces/about.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'portrait'],
        sizeToken: 'measure-compact',
        captureValue: 'Years list: 2-up at 40rem container, 3-up at 56rem.',
        wonder: 'The unique ratio is the years packing itself — a 4/5 still should show the turn, not one lonely year.',
    },
    {
        id: 'recipes-hook',
        label: 'Recipes hook',
        seat: 'hook',
        selector: '#recipe-field',
        specimenRoute: '/recipes/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'square', 'portrait'],
        captureValue: 'Kitchen entry spine. Mise and timing as a still.',
        wonder: 'A square card of mise is more honest than a full-page kitchen screenshot.',
    },
    {
        id: 'join-specimens',
        label: 'Workshop join specimens',
        seat: 'cluster',
        selector: '#join-crawl',
        specimenRoute: '/topics/software/spw/#join-crawl',
        cssOwner: 'public/css/systems/expression-resonance.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape'],
        captureValue: 'Four join kinds with living terms emphasized — identifier, crawl, common, project; a still of the plate is the lesson.',
        wonder: 'Cullet, mill, scrap should photograph as marks, not as plain prose.',
    },
    {
        id: 'boonhonk-theme-chips',
        label: 'Boonhonk theme chips',
        seat: 'cluster',
        selector: '#boonhonk-register .frame-operators[data-spw-feature="theme-chips"]',
        specimenRoute: '/about/#boonhonk-register',
        cssOwner: 'public/css/systems/expression-resonance.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Linked chip rail as a theme. Breadth is the row, not a paragraph.',
        wonder: 'A still of four chips should name recovered gold, mill, crew record, and quest without a mascot.',
    },
    {
        id: 'town-hook',
        label: 'Town atlas hook',
        seat: 'hook',
        selector: '#town-atlas-frame',
        specimenRoute: '/town/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Labeled atlas diagram, visit/world lens, landmark path, climate rail.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'A pocket crop should name library → card without the origin prose.',
    },
    {
        id: 'research-hook',
        label: 'Research hook',
        seat: 'hook',
        selector: '#research-frame',
        specimenRoute: '/research/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape'],
        captureValue: 'Cite/decide lens, insight ladder SVG, tropes rail.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'The ladder rungs should photograph as named marks, not a caption dump.',
    },
    {
        id: 'membership-hook',
        label: 'Membership hook',
        seat: 'hook',
        selector: '#membership-frame',
        specimenRoute: '/membership/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Read/join lens, role-field SVG, climate rail.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'Roles around the issue bed should be nameable in a still.',
    },
    {
        id: 'svg-storytelling-hook',
        label: 'SVG storytelling hook',
        seat: 'hook',
        selector: '#svg-storytelling',
        specimenRoute: '/topics/craft/svg/',
        cssOwner: 'public/css/systems/svg-surfaces.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape'],
        captureValue: 'Motif kit with glyph, creature, divider, token still readable.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'A still of the kit should name the four marks without the whole essay.',
    },
    {
        id: 'folio-fold-probe',
        label: 'Folio fold probe',
        seat: 'cluster',
        selector: '#folio-fold-probe',
        specimenRoute: '/design/folios/#folio-fold-probe',
        cssOwner: 'public/css/routes/surfaces/design.css',
        layoutScenarios: ['pocket', 'fold'],
        socialAspects: ['fit', 'square'],
        sizeToken: 'measure-card',
        captureValue: 'Rest/open radios on the kinetic study. One fold becomes a reveal.',
        chapter: 'physics',
        liminality: 'middle',
        wonder: 'The still should show the edge changing job, not a caption about components.',
    },
    {
        id: 'folio-growth-probe',
        label: 'Folio growth probe',
        seat: 'cluster',
        selector: '#folio-growth-probe',
        specimenRoute: '/design/folios/#folio-growth-probe',
        cssOwner: 'public/css/routes/surfaces/design.css',
        layoutScenarios: ['pocket', 'fold'],
        socialAspects: ['fit', 'square'],
        sizeToken: 'measure-card',
        captureValue: 'Rest/branch/fold/stop radios. Every new branch needs room.',
        chapter: 'physics',
        liminality: 'middle',
        wonder: 'A still of the organism should name the growth rule without looking like a biology sim.',
    },
    {
        id: 'folio-prices',
        label: 'Folio price ladder',
        seat: 'path',
        selector: '#folio-prices',
        specimenRoute: '/design/folios/#folio-prices',
        cssOwner: 'public/css/routes/surfaces/design.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'landscape'],
        sizeToken: 'measure-reading',
        captureValue: 'Planning asks $35 / $75 / $150, Seed 3-for-$95, prints quoted. Not live offers.',
        chapter: 'page',
        liminality: 'threshold',
        wonder: 'Amounts should be readable in a pocket crop without looking like checkout.',
    },
    {
        id: 'curriculum-hook',
        label: 'Curriculum hook',
        seat: 'hook',
        selector: '#curriculum-hero',
        specimenRoute: '/curriculum/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Creator-first title, living terms, three practice beds.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'One mechanism, one test, one artifact. Neighbors stay named.',
    },
    {
        id: 'software-hook',
        label: 'Software hook',
        seat: 'hook',
        selector: '#software-surface',
        specimenRoute: '/topics/software/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'CS half of the curriculum. Grammar, not a baker\'s-dozen hook.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'Named owners and the step-trace living term should survive a pocket crop.',
    },
    {
        id: 'math-hook',
        label: 'Math hook',
        seat: 'hook',
        selector: '#math-hero',
        specimenRoute: '/topics/math/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Structure atlas as the math neighbor.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'Dose and receipt stay countable in the opening still.',
    },
    {
        id: 'topics-register',
        label: 'Topics register',
        seat: 'hook',
        selector: '#topics-register',
        specimenRoute: '/topics/',
        cssOwner: 'public/css/systems/region-seats.css',
        layoutScenarios: ['pocket', 'fold', 'broadsheet'],
        socialAspects: ['fit', 'og'],
        captureValue: 'Atlas hook for the topic rooms.',
        chapter: 'region',
        liminality: 'entry',
        wonder: 'The register should name rooms, not a topic dump.',
    },
    {
        id: 'rpg-boonhonk',
        label: 'RPG boonhonk frame',
        seat: 'cluster',
        selector: '#rpgw-boonhonk',
        specimenRoute: '/play/rpg-wednesday/',
        cssOwner: 'public/css/effects/texture-slice.css',
        layoutScenarios: ['pocket', 'fold'],
        socialAspects: ['fit', 'square'],
        captureValue: 'Genre bible with harlequin slice behind playable copy.',
        chapter: 'climate',
        liminality: 'middle',
        wonder: 'Texture sits behind the mix, not on top of the ink.',
    },
]);
export function getRegionEcologyFixture(id) {
    return REGION_ECOLOGY_FIXTURES.find((fixture) => fixture.id === id);
}
