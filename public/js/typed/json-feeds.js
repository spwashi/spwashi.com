...
function validateMediaItem(value, path, issues) {
    if (!isRecord(value)) {
    ...
    if (!title)
        pushIssue(issues, `${path}.title`, 'title is required');
    if (!summary)
        pushIssue(issues, `${path}.summary`, 'summary is required');
    if (!href)
        pushIssue(issues, `${path}.href`, 'href is required');
    return Boolean(title && summary && href);
}
function validatePromoWonderCard(value, path, issues, role) {
    if (!isRecord(value)) {
        pushIssue(issues, path, 'expected an object');
        return false;
    }
    ...
    const href = asString(value.href);
    if (!title)
        pushIssue(issues, `${path}.title`, `${role} title is required`);
    if (!summary)
...
function validatePromoWonderPair(value, path, issues) {
    if (!isRecord(value)) {
        pushIssue(issues, path, 'expected an object');
        return false;
    }
...
export function validatePromoWonderFeed(value) {
    const issues = [];
    if (!isRecord(value)) {
...
export function validateMediaPublishingConfig(value) {
    const issues = [];
    if (!isRecord(value)) {
    ...
    for (const collectionKey of ['featuredPages', 'featuredTopics', 'featuredComponents']) {
...
export function parseJsonValue(text, label = 'json') {
    try {
...
export function formatJsonIssues(issues) {
