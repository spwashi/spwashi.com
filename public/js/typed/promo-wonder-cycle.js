...
const DEFAULT_FEED = Object.freeze({
    ...
    cycle: {
        promo: {
            label: 'What changed',
            operator: '@',
            title: 'Folio orders are open by card',
            summary: 'Fill the order card on Art and send a screenshot, the copied seed, or the downloaded .spw.txt. The reply names the piece and its price.',
            href: '/design/folios/#folio-request',
            cta: 'Open the order card',
            why: 'The card is the order form and the receipt. Working ranges are settled in the reply; commissions stay on the services ladder.',
            copyUnit: 'home.promoWonderCycle.cycle.promo',
            presentation: 'modal',
            promotion: {
                kind: 'service',
                audience: 'readers who have watched the folio stack return each season and want one piece from it',
                offer: 'A laminated folio, ordered by card: name the piece, the form, and the story it should carry',
                proof: 'Now lists the September 18 receipt; the order card on Art copies its seed, downloads .spw.txt, and screenshots clean.',
                objection: 'An order card should be the receipt, not a form that leads to another form.',
                urgency: 'Open now; the B-cycle closes September 26.',
                tone: 'clear',
                theme: 'glass',
                handles: ['folios', 'order', 'seed', 'cycle'],
                ctaStyle: 'primary',
                presentation: 'modal',
            },
        },
...
export function upcomingReleaseForecasts(date = new Date(), count = 2) {
    ...
    return candidates
        ...
        .map((candidate) => ({
        date: candidate,
        cycle: releaseCycleForDate(candidate),
...
function forecastDateLabel(date, locale, compact = false) {
    return date.toLocaleDateString(locale, compact
        ? { month: 'short', day: 'numeric' }
        : { weekday: 'long', month: 'long', day: 'numeric' });
}
function forecastTiming(forecast) {
    if (forecast.daysAway === 0)
        return `${forecast.cycle} closes today.`;
