...
const FRAME_SELECTOR = [
  'main > .site-frame[aria-labelledby]',
  'main > section.site-frame[aria-labelledby]',
  'main > article.site-frame[aria-labelledby]',
  '.site-frame[data-spw-role][aria-labelledby]'
].join(', ');
...
const normalizeCleanup = (cleanup) => (typeof cleanup === 'function' ? cleanup : null);
...
const initSiteCore = () => {
  ...
  const resolveTargetFrame = (selector) => {
    ...
    if (target.matches(FRAME_SELECTOR)) return target;
  ...
  const anchorSelector = '.frame-sigil[href^="#"], .operator-chip[href^="#"], a[href^="#"]';
  const frameContextSelector = [
    '.frame-card',
    '.operator-card',
    '.software-card',
    '.media-card',
  ...
  document.addEventListener('click', (event) => {
    ...
    const targetFrame = resolveTargetFrame(href);
    if (targetFrame) {
  ...
  const skipLinks = Array.from(document.querySelectorAll('a.skip-link[href^="#"]'));
  skipLinks.forEach((link) => {
    link.addEventListener('click', () => {
      const target = resolveTargetFrame(link.getAttribute('href'));
      if (target) {
        activateFrame(target, { source: 'skip-link', force: true });
      }
    }, { signal: abortController.signal });
  });

  const groupedNodes = Array.from(document.querySelectorAll('[data-mode-group]'));
  const groupNames = Array.from(new Set(groupedNodes.map((node) => node.dataset.modeGroup).filter(Boolean)));
  ...
  groupNames.forEach((groupName) => {
    ...
    const initialMode =
      buttons.find((button) => button.getAttribute('aria-pressed') === 'true')?.dataset.setMode ||
  ...
  if ('IntersectionObserver' in window && frames.length > 1) {
    ...
    const syncViewportFrame = () => {
      ...
      const visibleCandidates = frames
        .filter((frame) => (visibleFrames.get(frame) || 0) > 0)
        .map((frame) => ({
          frame,
      ...
      const nextFrame = visibleCandidates[0]?.frame;
      if (!nextFrame || nextFrame === state.activeFrame) return;

      activateFrame(nextFrame, { source: 'viewport' });
    };
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        visibleFrames.set(entry.target, entry.isIntersecting ? entry.intersectionRatio : 0);
      });
      scheduleViewportSync();
    }, {
    ...
    window.addEventListener('scroll', throttledViewportSync, {
      passive: true,
      signal: abortController.signal
    });
    ...
    observerCleanups.push(() => {
      if (viewportSyncId) cancelAnimationFrame(viewportSyncId);
...
const CORE_MODULES = [
  ...
  },
  {
    ...
    mindfulness: {
      level: 'site',
      broaderPatterns: ['frame', 'mode-switch', 'hash', 'viewport', 'landmark']
    },
    mount: () => initSiteCore()
  },
  {
...
const OPTIONAL_FEATURES = [
  ...
  },
  {
    ...
    scope: 'site',
    optional: true,
    mindfulness: { level: 'site', broaderPatterns: ['component-semantics', 'structure'] },
    when: () => true
  }
];
